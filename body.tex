...
\LPG[inline]{My reading of this timeline is that we want to have all the sections that do not depend on commissioning data written and reviewed by Feb. 2021. If this is so, I'd add an introductory sentence to explicitly state this.}
...
\item 
Delivery of paper outlines and the second telecon (Feb 2020) \LPG[inline]{Suggest Jan. 2020 - the outline should not require 4 months.  I'd also add here: names for lead author for each of these sections,  and a  sentence or 2 on the on the section scope.}
\item
...
\LPG[inline]{Suggest to specify more clearly what is expected in a rough draft, e.g  a list of tables, figures, subsection structure,  rough text,  and identification of any impediments to make the Oct. deadline for drafts ready for review so that we can replan if need be.  Essentially we  are giving Jan - Oct to write the non-data sections with a mid-way checkpoint  of June. }
\item 
Sections that can be written without having the data ready for review and
the fourth telecon (Oct 2020) \LPG[inline]{I assume this is for internal project review? If so, update to say internal review. I'd also push to Nov. 2020. 3 months should be sufficient for reviewers. The Jun. - Oct. period is typically busy with meetings/reviews and writers will have more conflicts.}
\item
...
\item
Implementation of the reviewers' comments (from Aug 2021 \LPG{Aug. 2022?} until the end of construction)
